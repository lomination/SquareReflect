public static class ConsoleTileConverter {
    public static ConsoleTile ConvertTile(Tile tile) {
        return tile.GetType().Name switch {
            ...
            nameof(FragileAngle) => new ConsoleTile(tile.Clone(), t => ((FragileAngle)t).Count > 0 ? "◣◤◥◢"[(int)((FragileAngle)tile).Dir].ToString() : " "),
            nameof(GhostAngle) => new ConsoleTile(tile.Clone(), t => ((GhostAngle)t).Count > 0 ? " " : "◣◤◥◢"[(int)((GhostAngle)tile).Dir].ToString()),
